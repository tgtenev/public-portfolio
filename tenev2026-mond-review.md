...
**The Baryonic Tully–Fisher Relation (BTFR).** The flat velocity's fourth power is proportional to the galaxy's *baryonic* mass, stars plus gas, with a proportionality constant fixed by $a_0 G$ and by nothing else. On a log-log plot (both axes logarithmic, so a power law $M_\mathrm{bar} \propto v_f^{\ n}$ appears as a straight line of slope $n$), equation (4) predicts a single line of slope exactly 4, the same line for every galaxy, with no allowance for a galaxy's size or how concentrated its light is. That the relation involves total baryonic mass rather than starlight matters most in gas-rich dwarf galaxies, where stars are a minority of the baryons: there, the older luminosity-based Tully–Fisher relation falls apart while the baryonic version holds (Section 3.3).
...
The harvest extends beyond those two. Famaey and McGaugh [3] collect ten distinct "Kepler-like laws of galactic dynamics" that the single constant $a_0$ organizes at once. Among them: the asymptotic flatness and the BTFR above; the analogous velocity–mass relation for elliptical galaxies, which are supported by random stellar motions rather than rotation (the Faber–Jackson relation); a systematic relation between the size of a galaxy's mass discrepancy and its acceleration; a maximum characteristic surface density of galaxy disks, set by $a_0/G$; a split in rotation-curve shapes (rising versus flat-from-the-start) at that same surface-density scale; and the rule, named for the astronomer Renzo Sancisi, that every feature in a galaxy's baryonic profile (a bump in the light, a ring of gas) has a counterpart feature in its rotation curve, even far out where dark matter would supposedly dominate completely. Their summary of that list states the situation plainly: ten otherwise-independent regularities "have been elegantly unified by the single law proposed by Milgrom." Whatever one concludes about MOND as a theory, that unification is a fact about galaxies.

## 3. The modern evidence

### 3.1 SPARC: the database

Modern work on these questions rests on **SPARC** (Spitzer Photometry and Accurate Rotation Curves; Lelli, McGaugh and Schombert [6]): a sample of 175 disk galaxies with high-quality rotation curves and uniform photometry from the Spitzer Space Telescope in the near-infrared, at a wavelength of 3.6 μm. The sample spans five decades of mass, every disk-galaxy type, gas fractions from 0.01 to 0.97, and the full observed range of surface brightness (how spread out or concentrated a galaxy's light is).

The near-infrared choice is the methodological centerpiece. Starlight at 3.6 μm is dominated by the old stars that carry most of the stellar mass, and models of stellar populations predict that in this band the mass-to-light ratio $\Upsilon_\star$ is nearly the *same* for every disk galaxy, about 0.5 in solar units. Converting light to mass therefore needs **one universal number**, not a per-galaxy adjustment. That removes, at the source, the single largest freedom that plagued earlier work (Section 3.4).
...
![Figure 2: the Radial Acceleration Relation — thousands of points from about 150 galaxies collapsing onto one one-parameter curve](https://figures.tgtenev.com/tenev2026-mond-review/fig02-rar.svg)

**Figure 2.** The Radial Acceleration Relation, rebuilt from the public SPARC database with the quality cuts of the source analysis (disk inclination at least 30°, the two best quality classes, velocities measured to 10% or better; 2696 points appear here, against the published 2693 across 153 galaxies, the small difference coming from an approximated sample cut). Every point is one radius in one galaxy. The red curve is equation (5) with the published $g_\dagger$, not refit to this rendering. Dotted: the deep-limit square-root branch. Dashed: the line of equality.
...
![Figure 4: the External Field Effect — an isolated dwarf keeps its deep-MOND boost; the same dwarf embedded in a host's field becomes quasi-Newtonian](https://figures.tgtenev.com/tenev2026-mond-review/fig04-efe.svg)
...
### 4.5 The Solar System: no anomaly to explain

It is worth stating plainly what the nearest and best-measured gravitational system contributes, because two independent lines of evidence settle the question here, and the result is sometimes misread.

The first is the ephemerides of the previous subsection, read now as a measurement rather than as a constraint on $\mu$. Pitjeva and Pitjev [17] fit the planetary orbits together with decades of spacecraft ranging to a model containing only the known masses, and find no residual acceleration of the kind either a dark component or a modified force law would introduce. The same fits bound any smoothly distributed dark mass inside Saturn's orbit far below the level that would perturb the planets detectably.

The second is the **Pioneer anomaly**, for two decades the most-cited candidate for new gravitational physics at an interesting acceleration. Tracking of Pioneer 10 and 11 beyond about 20 AU showed an unmodeled sunward acceleration of $(8.74 \pm 1.33)\times10^{-10}\ \mathrm{m\ s^{-2}}$ (Anderson et al. [18]). That value sits within a factor of two of $cH_0$ and about seven times $a_0$, close enough to the scales of Section 5 that a connection to modified-dynamics physics was widely canvassed, although MOND itself predicts no departure from Newton at those distances. The anomaly did not survive scrutiny. Turyshev et al. [19] modeled the recoil from the spacecraft's own waste heat, the radioisotope generators' thermal emission leaving the vehicle anisotropically after reflecting off the antenna dish, and accounted for the entire signal, including its slow decline over the mission, which follows the decay of the plutonium fuel. The Pioneer anomaly was a thermal engineering effect, not a gravitational one.

Taken together these give a firm negative result: **the dark-matter effect is not observed in the Solar System, in either of the two forms it could take.** There is no unseen mass, and there is no departure from Newtonian gravity.

MOND's handling of that negative result is a consistency requirement rather than a prediction, and the distinction matters. MOND is a threshold phenomenon: the law changes below $a_0$, and the Solar System is nowhere near the threshold. The radius at which the Sun's own gravity falls to $a_0$ is $\sqrt{GM_\odot/a_0} \approx 7000$ AU, about 0.03 pc, which is more than two hundred times Neptune's orbital radius and roughly a hundred times the distance Pioneer was tracked to; it lies out in the Oort cloud. Over Pioneer's 20 to 70 AU the Sun pulls with $10^{-6}$ to $10^{-5}\ \mathrm{m\ s^{-2}}$, four to five decades above $a_0$; at Saturn the margin is nearly six decades. MOND therefore predicts ordinary Newtonian dynamics throughout the region where all of these measurements were made, and the null results are what it expects rather than a difficulty it must absorb.

Two qualifications keep this from being a free pass. The predicted null holds only for interpolation functions whose high-acceleration tail dies away fast enough, which is exactly the constraint of Section 4.4: the "simple" function taken literally does not pass, so the ephemerides do real work in discriminating among MOND's admissible forms even though they cannot probe the low-acceleration regime at all. And the null does not discriminate between frameworks. The standard cosmology likewise predicts a negligible dark-matter contribution here, since the local halo density, of order $0.01$ solar masses per cubic parsec, amounts to a vanishing mass within planetary orbits. Both accounts pass; the Solar System is simply not where they differ. The regime that does separate them is a system with accelerations near $a_0$ but too small to carry a halo of its own, which is the wide-binary test of Section 6.4.
...
![Figure 6: the acceleration number line — all Solar-System accelerations sit far above the MOND scale, which lands within an order-one factor of cH0](https://figures.tgtenev.com/tenev2026-mond-review/fig06-a0-coincidence.svg)

**Figure 6.** The scale and the coincidence. Every laboratory and Solar-System acceleration sits far above the MOND scale, from nearly six decades above it at Saturn to about eleven at Earth's surface; the anomaly begins where gravity is about as weak as the acceleration that takes an object from rest to the speed of light over the age of the universe. The ratio $c H_0/a_0 \approx 6$ is an order-one number: the galactic acceleration scale appears to know about cosmology.
...
| Test | Standing | Note |
|---|---|---|
| Solar-System dynamics | successful | no anomaly to explain; ephemerides and the resolved Pioneer anomaly (Section 4.5) |
...
| Baryonic Tully–Fisher relation | successful | slope 4, low scatter, gas-rich anchor (Section 3.3) |
...
Ultra-diffuse galaxies (galaxies as large as giants but exceedingly dim) cut both ways. The satellites NGC 1052-DF2 and DF4 show internal motions consistent with *no* dark matter at all (van Dokkum et al. [23]). A halo can plausibly be stripped away by tides; a law of nature cannot be switched off, so MOND must attribute these systems' Newtonian behavior to the EFE of the nearby giant NGC 1052, an explanation whose adequacy is debated. In the opposite direction, the isolated gas-rich dwarf AGC 114905 shows a nearly Newtonian rotation curve where MOND predicts a large boost and no external field can be blamed (Mancera Piña et al. [24]); here the contested quantity is the galaxy's inclination, which the discrepancy claim depends on sensitively. Both cases illustrate the same methodological point: the EFE gives MOND real explanatory reach and real vulnerability to the charge of unfalsifiability, at the same time.

### 6.4 A test in progress: wide binary stars

Pairs of stars orbiting each other at separations wider than a few thousand astronomical units (AU, the Earth–Sun distance) have mutual gravitational accelerations that approach and fall below $a_0$. They are a clean discriminating test, because nobody proposes that dark matter clumps inside a stellar binary: Newtonian gravity predicts ordinary Keplerian motion there, while MOND (with the Milky Way's external field properly included via the EFE) predicts relative velocities enhanced by roughly 15–20%. The European Space Agency's *Gaia* satellite, which measures positions and motions for about two billion stars, supplies exactly the needed data.

The field is in open conflict. Chae [25] and, independently, Hernandez and collaborators report a velocity enhancement consistent with the MOND expectation, across several analyses. Banik et al. [26] analyze a differently selected sample and report a strong null result, quoted near 19σ against the MOND prediction. Each camp disputes the other's sample selection, treatment of unseen third companions (a hidden close companion inflates the apparent relative velocity), and statistical modeling, rather than the underlying *Gaia* measurements. The dispute is genuinely unresolved: treat wide binaries as a test in progress, not as a verdict for either side.
...
![Figure 7: two synthetic acceleration-relation panels — the scattered cloud produced if each galaxy's halo depended on its own history, versus the observed tight relation](https://figures.tgtenev.com/tenev2026-mond-review/fig07-tightness.svg)
...
- The dark-matter effect is not observed in the Solar System, in either form: planetary ephemerides show no anomalous residual and no unseen mass [17], and the Pioneer anomaly, long the leading candidate for new gravitational physics at a comparable acceleration, was accounted for by thermal recoil from the spacecraft [18, 19]. This is consistent with MOND, whose threshold lies near 7000 AU from the Sun, but it does not discriminate: the standard cosmology predicts a negligible local dark-matter contribution too (Section 4.5).
- The scale itself is the deepest clue: $a_0 \approx cH_0/2\pi$ within tens of percent, tying galaxy dynamics to cosmology for no known reason. Whether the scale is frozen (a $\Lambda$-like reading) or tracks cosmic history (an $H$-like reading) is testable against high-redshift galaxies [20].
- The record is scale-dependent: MOND is repeatedly successful in disk galaxies, contested in wide binaries and ultra-diffuse dwarfs, and fails by a factor of about 2 in galaxy clusters, where unseen mass is reduced but not eliminated [3, 21, 22]. Relativistic completions exist; the simple ones are ruled out by the equality of gravitational-wave and light speeds, and the surviving one (AeST [29]) fits the CMB by construction rather than by explanation.
...
[1] M. Milgrom, "A modification of the Newtonian dynamics as a possible alternative to the hidden mass hypothesis," Astrophys. J., vol. 270, pp. 365–370, 1983.

[2] M. Milgrom, "A modification of the Newtonian dynamics — Implications for galaxies," Astrophys. J., vol. 270, pp. 371–383, 1983; and companion, pp. 384–389, 1983.
...
[5] A. Bosma, "21-cm line studies of spiral galaxies. II," Astron. J., vol. 86, pp. 1825–1846, 1981.
...
[9] F. Lelli, S. S. McGaugh, and J. M. Schombert, "The small scatter of the baryonic Tully–Fisher relation," Astrophys. J. Lett., vol. 816, L14, 2016.

[10] A. A. Dutton, "The baryonic Tully–Fisher relation and galactic outflows," Mon. Not. R. Astron. Soc., vol. 424, pp. 3123–3128, 2012.
...
[13] J. Bekenstein and M. Milgrom, "Does the missing mass problem signal the breakdown of Newtonian gravity?" Astrophys. J., vol. 286, pp. 7–14, 1984.

[14] M. Milgrom, "Quasi-linear formulation of MOND," Mon. Not. R. Astron. Soc., vol. 403, pp. 886–895, 2010.
...
[16] B. Famaey and J. Binney, "Modified Newtonian dynamics in the Milky Way," Mon. Not. R. Astron. Soc., vol. 363, pp. 603–608, 2005.

[17] E. V. Pitjeva and N. P. Pitjev, "Relativistic effects and dark matter in the Solar system from observations of planets and spacecraft," Mon. Not. R. Astron. Soc., vol. 432, pp. 3431–3437, 2013.

[18] J. D. Anderson, P. A. Laing, E. L. Lau, A. S. Liu, M. M. Nieto, and S. G. Turyshev, "Study of the anomalous acceleration of Pioneer 10 and 11," Phys. Rev. D, vol. 65, 082004, 2002; see also Phys. Rev. Lett., vol. 81, pp. 2858–2861, 1998.

[19] S. G. Turyshev, V. T. Toth, G. Kinsella, S.-C. Lee, S. M. Lok, and J. Ellis, "Support for the thermal origin of the Pioneer anomaly," Phys. Rev. Lett., vol. 108, 241101, 2012.

[20] R. Genzel et al., "Strongly baryon-dominated disk galaxies at the peak of galaxy formation ten billion years ago," Nature, vol. 543, pp. 397–401, 2017.

[21] R. H. Sanders and S. S. McGaugh, "Modified Newtonian dynamics as an alternative to dark matter," Annu. Rev. Astron. Astrophys., vol. 40, pp. 263–317, 2002.

[22] D. Clowe et al., "A direct empirical proof of the existence of dark matter," Astrophys. J. Lett., vol. 648, L109, 2006.

[23] P. van Dokkum et al., "A galaxy lacking dark matter," Nature, vol. 555, pp. 629–632, 2018; and Astrophys. J. Lett., vol. 874, L5, 2019.

[24] P. E. Mancera Piña et al., "No need for dark matter: resolved kinematics of the ultra-diffuse galaxy AGC 114905," Mon. Not. R. Astron. Soc., vol. 512, pp. 3230–3242, 2022.

[25] K.-H. Chae, "Breakdown of the Newton–Einstein standard gravity at low acceleration in internal dynamics of wide binary stars," Astrophys. J., vol. 952, 128, 2023.

[26] I. Banik et al., "Strong constraints on the gravitational law from Gaia DR3 wide binaries," Mon. Not. R. Astron. Soc., vol. 527, pp. 4573–4615, 2024.

[27] J. D. Bekenstein, "Relativistic gravitation theory for the modified Newtonian dynamics paradigm," Phys. Rev. D, vol. 70, 083509, 2004.

[28] B. P. Abbott et al., "Gravitational waves and gamma-rays from a binary neutron star merger: GW170817 and GRB 170817A," Astrophys. J. Lett., vol. 848, L13, 2017.

[29] C. Skordis and T. Złośnik, "New relativistic theory for modified Newtonian dynamics," Phys. Rev. Lett., vol. 127, 161302, 2021.

[30] A. D. Ludlow et al., "Mass-discrepancy acceleration relation: A natural outcome of galaxy formation in cold dark matter halos," Phys. Rev. Lett., vol. 118, 161103, 2017; see also A. Di Cintio and F. Lelli, Mon. Not. R. Astron. Soc., vol. 456, L127, 2016.

---

*A note on the figures.* Figures 1–3 are built from the public SPARC database [6] with the sample cuts stated in their captions; Figures 4–7 are original diagrams. No figure is reproduced from a published paper. Where this review and a primary source disagree, the source is correct.
